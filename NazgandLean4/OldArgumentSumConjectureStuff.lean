...
-- (DiffEqCoeff : (Fin (n + 2)) → ℂ) -- (n + 1) coefficients of the differential equation

def LeadCoeffNonZero {n : ℕ+} (DiffEqCoeff : (Fin (n + 2)) → ℂ) : Prop
  := DiffEqCoeff (Fin.ofNat (n + 2) (n + 1)) ≠ 0 -- needed to keep the degree equal to n

def IsDifferentialEquationSolution {n : ℕ+} (DiffEqCoeff : (Fin (n + 2)) → ℂ) (f : ℂ → ℂ) : Prop
  := --checks whether a given function is a solution
  ContDiff ℂ ⊤ f ∧ ∀ (z : ℂ), 0 = ∑ (k : Fin (n + 2)) , (DiffEqCoeff k) * (iteratedDeriv k f z)

def SetOfSolutions {n : ℕ+} (DiffEqCoeff : (Fin (n + 2)) → ℂ) : Set (ℂ → ℂ) :=
  {h : ℂ → ℂ | IsDifferentialEquationSolution DiffEqCoeff h}
...
theorem BasisMatrixImageOfBasis {n : ℕ+} {DiffEqCoeff : (Fin (n + 2)) → ℂ} (_ : LeadCoeffNonZero DiffEqCoeff)
  ...
  have h₃ : ∀ k : Fin (n + 1), g₀ k ∈ SetOfSolutions DiffEqCoeff := by
    ...
    use (λ k₀ : Fin (n + 1) => if k = k₀ then (1 : ℂ) else (0 : ℂ))
...
theorem AsymmArgumentSumConjecture {n : ℕ+} {DiffEqCoeff : (Fin (n + 2)) → ℂ}
  ...
  have h₅ := congrArg (λ B : Matrix (Fin (n + 1)) (Fin 1) ℂ => Bᵀ) (hC z₀)
