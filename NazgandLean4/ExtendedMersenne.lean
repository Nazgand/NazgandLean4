  ...
  | succ m =>
    ...
    have hk : k ≥ 2 := by
      ...
      | succ k' =>
        cases k' with
        | zero =>
          simp only [ExtendedMersenne, zero_add, Finset.range_one, Finset.sum_singleton, pow_zero] at hp
    ...
    have h2 : ExtendedMersenne ((m + 1) ^ x) y > 1 :=
      ExtendedMersenneGt1IfBGt0KGt1 _ _ (Nat.pow_pos (Nat.succ_pos m)) (by linarith [hy2])
...
theorem ExtendedMersenneGcdK (b k₀ k₁ : ℕ) :
  ...
  | zero =>
    ...
    · simp only [↓reduceIte, Nat.add_eq_zero_iff, one_ne_zero, and_false, Nat.gcd_one_right]
    · simp only [Nat.add_eq_zero_iff, one_ne_zero, and_false, ↓reduceIte, Nat.gcd_zero_right,
      and_true]
    · simp only [Nat.add_eq_zero_iff, one_ne_zero, and_false, ↓reduceIte, Nat.gcd_self, and_self]
  | succ m =>
    cases m with
    | zero =>
